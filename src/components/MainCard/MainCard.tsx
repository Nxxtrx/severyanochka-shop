...
const MainCard: FC<ProductType> = ({ cardImage, title, price, name, rating, item }) => {
  ...
  const handleToggleFavorite = (foodItem: IFood): void => {
  ...
  const findFavoriteFood = (id: string): boolean => {
  ...
  return (
    <li className="main-cards__card">
      <button
        className={`main-cards__like ${isLike || item.isLike ? 'main-cards__like_type_active' : ''}`}
        onClick={() => {
          handleToggleFavorite(item);
        }}></button>
      <img className="main-cards__image" src={cardImage} alt="" />
      {title === 'sale' && (
        <div className="main-cards__sale">
          <span>-50%</span>
        </div>
      )}
      <div className="main-cards__price-container">
        <p className="main-cards__price">
          {price} ₽{' '}
          {title === 'Акции' && <span className="main-cards__price-description">С картой</span>}
        </p>
        {title === 'sale' && (
          <p className="main-cards__price main-cards__price_type_usually">
            {price * 0.5} ₽ <span className="main-cards__price-description">Обычная</span>
          </p>
        )}
      </div>
