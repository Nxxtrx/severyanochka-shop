...
const CartTotal: FC = () => {
  ...
  const calculateBonusAmount = (): number => {
    const finalAmount = total - sale;
    if (check && finalAmount <= bonus) {
  ...
  return (
    <div className="total">
      ...
      <div className="total__sum-container">
        <p>
          {count} товара<span>{total} ₽</span>
        </p>
        {sale > 0 && (
          <p>
            Скидка<span className="total__sale">-{sale} ₽</span>
          </p>
        )}
        {check && (
          <p>
            Бонусы
            <span className="total__sale">-{calculateBonusAmount()} ₽</span>
          </p>
        )}
      </div>
      <p className="total__sum-total">
        Итог <span>{calculateFinalAmount()} ₽</span>
      </p>
      <p className="total__take-bonus">
        <img src={smileImage} alt="" />
        Вы получаете <span>&nbsp;100 бонусов</span>
      </p>
      {calculateFinalAmount() <= 999 && (
        <div className="total__min-sum">Минимальная сумма заказа 1000р</div>
      )}
      <button className="total__btn" disabled={calculateFinalAmount() <= 999}>
        Оформить заказ
      </button>
    </div>
  );
